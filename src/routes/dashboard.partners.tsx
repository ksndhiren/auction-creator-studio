...
export const Route = createFileRoute("/dashboard/partners")({
  head: () => ({ meta: [{ title: "Partners — Auction Creative Studio" }] }),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <div className="text-xs uppercase tracking-[0.3em] text-muted-foreground">Team</div>
          <h1 className="text-stencil text-3xl">Partners</h1>
          <p className="text-muted-foreground">Invite auction partners and manage access.</p>
        </div>
          ...
          <tbody>
            {partners.map((p) => (
              <tr key={p.id} className="border-t border-border">
                ...
                <td className="px-5 py-4">
                  <span className={`px-2 py-1 text-[10px] font-bold uppercase tracking-widest ${statusColor[p.status]}`}>
                    {p.status}
                  </span>
                </td>
